...
4.  **Audit Drill-Down**:
    ...
    - **Exposure Tracking**: The system tracks exposure in real-time. "Issued" LCs contribute to **Firm Exposure**, while "Pending" or "Draft" transactions contribute to **Contingent/Reserved Exposure**.
    ...
    - **Settlement Impact**: The facility limit is released only upon **Authorization** of a settlement transaction (not just initiation). This ensures that the bank's exposure is only reduced once the payment is legally committed.
    - **Unified Narrative Timeline**: Review a merged view of financial transactions and system audit logs. Every business action (Issuance, Amendment, Settlement) is linked to a unique **Transaction Ref** and tracked via the `latestTransactionId` on the instrument for high-fidelity audit chaining.
