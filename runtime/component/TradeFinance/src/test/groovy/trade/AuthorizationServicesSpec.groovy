...
class AuthorizationServicesSpec extends Specification {
    ...
    def setupSpec() {
        ...
        testPrefix = "AUTH-TEST-" + System.currentTimeMillis()
        cleanData()
    }
    ...
    private void cleanData() {
        ...
        try {
            ...
            ec.entity.find("trade.UserAuthorityProfile").condition("userAuthorityId", EntityCondition.LIKE, testPrefix + "%").deleteAll()
    ...
    def "Test Maker/Checker matrix prohibits self-approval"() {
        ...
        ec.service.sync().name("trade.importlc.ImportLcServices.create#ImportLetterOfCredit")
                .parameters([instrumentId: testPrefix + "1", instrumentRef: "TF-AUTH-01", businessStateId: "LC_DRAFT",
        ...
        def tx = ec.entity.find("trade.TradeTransaction").condition("instrumentId", testPrefix + "1").one()
        tx.setAll([transactionStatusId: 'TX_PENDING', makerUserId: 'john.doe']).update()
        
        ec.entity.makeValue("trade.TradeTransactionAudit")
            .setAll([transactionId:tx.transactionId, instrumentId:testPrefix + "1", auditId:testPrefix + "AUD-1", actionEnumId:"MAKER_COMMIT", userId:"john.doe"]).create()
        ec.entity.makeValue("trade.UserAuthorityProfile")
            .setAll([userAuthorityId:testPrefix + "T1-02", userId:"jane.doe", delegationTierId:"TIER_1", customLimit:100000.0, makerCheckerFlag: "CHECKER"]).create()
            
        when: "Checker is the same user"
        def resultSame = ec.service.sync().name("trade.AuthorizationServices.evaluate#MakerCheckerMatrix")
            .parameters([instrumentId:testPrefix + "1", userId:"john.doe"]).call()
        ...
        def resultDiff = ec.service.sync().name("trade.AuthorizationServices.evaluate#MakerCheckerMatrix")
            .parameters([instrumentId:testPrefix + "1", userId:"jane.doe"]).call()
    ...
    def "BDD-CMN-AUTH-03: Tier Routing uses effectiveAmount for amendments"() {
        ...
        ec.service.sync().name("trade.importlc.ImportLcServices.create#ImportLetterOfCredit")
                .parameters([instrumentId: testPrefix + "AMD", instrumentRef: "TF-AUTH-03", businessStateId: "LC_ISSUED", 
                             lcAmount: 150000.0, lcCurrencyUomId: 'USD',
                             instrumentParties: [[roleEnumId: 'TP_APPLICANT', partyId: 'ACME_CORP_001'],
                                                  [roleEnumId: 'TP_BENEFICIARY', partyId: 'GLOBAL_EXP_002']]]).call()
        
        def tx = ec.entity.find("trade.TradeTransaction").condition("instrumentId", testPrefix + "AMD").one()
        tx.setAll([transactionStatusId: 'TX_PENDING', makerUserId: 'maker.user']).update()
            
        and: "Audit record for maker"
        ec.entity.makeValue("trade.TradeTransactionAudit")
            .setAll([transactionId:tx.transactionId, instrumentId:testPrefix + "AMD", auditId:testPrefix + "AUD-2", actionEnumId:"MAKER_COMMIT", userId:"maker.user"]).create()
            
        and: "Checker with Tier 1 limit (100k)"
        ec.entity.makeValue("trade.UserAuthorityProfile")
            .setAll([userAuthorityId:testPrefix + "T1-03", userId:"tier1.user", delegationTierId:"TIER_1", customLimit:100000.0, makerCheckerFlag: "CHECKER"]).create()
        ...
        def result = ec.service.sync().name("trade.AuthorizationServices.evaluate#MakerCheckerMatrix")
            .parameters([instrumentId:testPrefix + "AMD", userId:"tier1.user"]).call()
        ...
        cleanup:
        if (ec != null) ec.artifactExecution.disableAuthz()
        ec.entity.find("trade.UserAuthorityProfile").condition("userAuthorityId", testPrefix + "T1-03").deleteAll()
        ec.entity.find("trade.TradeInstrument").condition("instrumentId", testPrefix + "AMD").updateAll([latestTransactionId: null])
        ec.entity.find("trade.importlc.SwiftMessage").condition("instrumentId", testPrefix + "AMD").deleteAll()
        ec.entity.find("trade.TradeTransactionAudit").condition("instrumentId", testPrefix + "AMD").deleteAll()
        ec.entity.find("trade.TradeTransaction").condition("instrumentId", testPrefix + "AMD").deleteAll()
        ec.entity.find("trade.importlc.ImportLetterOfCredit").condition("instrumentId", testPrefix + "AMD").deleteAll()
        ec.entity.find("trade.TradeInstrumentParty").condition("instrumentId", testPrefix + "AMD").deleteAll()
        ec.entity.find("trade.TradeInstrument").condition("instrumentId", testPrefix + "AMD").deleteAll()
    }

    def "BDD-CMN-AUTH-05: Priority Queue ordering sorts Urgent before Low"() {
        given: "Two instruments with different priorities"
        ec.service.sync().name("trade.importlc.ImportLcServices.create#ImportLetterOfCredit")
                .parameters([instrumentId: testPrefix + "LOW", instrumentRef: testPrefix + "-PRIO-01", businessStateId: "LC_DRAFT", priorityEnumId: "PRIO_LOW",
                             lcAmount: 1000.0, lcCurrencyUomId: 'USD',
                             instrumentParties: [[roleEnumId: 'TP_APPLICANT', partyId: 'ACME_CORP_001'],
                                                   [roleEnumId: 'TP_BENEFICIARY', partyId: 'GLOBAL_EXP_002']]]).call()
        
        ec.service.sync().name("trade.importlc.ImportLcServices.create#ImportLetterOfCredit")
                .parameters([instrumentId: testPrefix + "HIGH", instrumentRef: testPrefix + "-PRIO-02", businessStateId: "LC_DRAFT", priorityEnumId: "PRIO_URGENT",
                             lcAmount: 1000.0, lcCurrencyUomId: 'USD',
                             instrumentParties: [[roleEnumId: 'TP_APPLICANT', partyId: 'ACME_CORP_001'],
                                                   [roleEnumId: 'TP_BENEFICIARY', partyId: 'GLOBAL_EXP_002']]]).call()
        
        when: "Retrieving the list filtered by testPrefix"
        def result = ec.service.sync().name("trade.importlc.ImportLcServices.get#ImportLetterOfCreditList")
            .parameters([instrumentSearch: testPrefix]).call()
        def list = result.lcList
        
        then: "Urgent (PRIO-HIGH) should be before Low (PRIO-LOW)"
        int highIdx = list.findIndexOf { it.instrumentId == testPrefix + "HIGH" }
        int lowIdx = list.findIndexOf { it.instrumentId == testPrefix + "LOW" }
        highIdx != -1
        lowIdx != -1
        highIdx < lowIdx
    }
